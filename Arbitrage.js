...
const provider = new ethers.providers.JsonRpcProvider('key here');
...
const main = async () => {
    ...
    const uniPrice = Number(uniAmount[1]) / (uniAmount[0])
    ...
    if (uniPrice > sushiPrice) {
        ...
        if (spread > 0) {
            console.log('buy on uni, sell on sushi')
        } else {
            console.log('no arb oppurtunity')
        }
    }
}
